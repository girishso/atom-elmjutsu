...
function getHintTypeInfo(hint, activeFile) {
  if (atom.config.get('elmjutsu.showTypesInSidekick')) {
    ...
    const maybeArgs = hint.args.length > 0 ? ' ' + hint.args.join(' ') : '';
    // hint.tipe === ''
    //   ? hint.args.length > 0 ? ' ' + hint.args.join(' ') : ''
    //   : '';

    console.log('hint', hint);
    // "(a -> b) -> List.List a -> List.List b"  ====> "(a -> b) -> List a -> List b"
    hint.tipe = hint.tipe.replace(/(\w*[^\.])\.\1/g, '$1');
    const isRecord = hint.tipe.match(/\{/);
