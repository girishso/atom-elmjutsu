      ...
      render() {
        ...
        const elements = items.map(
          (
            ...
            const maybeArgsView =
              maybeArgs == '' ? null
                : hint.caseTipe === null ? <div>{maybeModuleName}<strong>{maybeName}</strong> <em>{maybeArgs}</em> =</div> // function args
                : null

            // if "type {alias}" has no args skip the Module.Name
            const typeOrArgsFirst = hint.kind.startsWith("type") && maybeArgs != '' ? <span>{maybeArgsView}{maybeTipeView}</span>
                : <span>
                    {maybeModuleName}
