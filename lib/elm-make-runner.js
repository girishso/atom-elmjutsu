...
function startWatching(path, callback, timeout) {
...
export default class ElmMakeRunner {
  ...
  compile(editor, usingLinterV1) {
    ...
    return new Promise(resolve => {
      ...
      //   })
        let fsWait = false;
        let path = `${projectDirectory}/report.json`
        return fs.watch(path, (event, filename) => {
          if (filename) {
            ...
            console.log(`${filename} file Changed`);
            return resolve(fs.readFileSync(path, "utf8"));
          }
        });
        // startWatching(`${projectDirectory}/report.json`, (txt) => {
        //   console.log("report.json changed!!!")
        //   return resolve(txt)
        // }
        )
        .then(data => {
