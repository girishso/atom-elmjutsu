...
async function mylint(reportJsonPath) {
  return fs.existsSync(reportJsonPath) ? watchForChange(reportJsonPath) : atomLinter
...
async function watchForChange(filePath) {
  return new Promise((resolve) => {
    if (window.reportWatched) {
      setTimeout(() => {
          resolve({
            stdout: "",
            stderr: fs.readFileSync(filePath, "utf8"),
            exitCode: 1,
          })
        })
        return
    ...
    let fsWait = false
    window.reportWatched = true
    let watcher = fs.watch(dir, (event, filename) => {
...
export default class ElmMakeRunner {
  ...
  compile(editor, usingLinterV1) {
    ...
    const execLocalOption =
      ...
      executablePath ===
        ...
        : undefined;
    const reportJsonPath = `${projectDirectory}/dist/report.json`

    return new Promise(resolve => {
        // mylint(reportJsonPath)
      (async function() {
          if(fs.existsSync(reportJsonPath)) {
            console.log('  >>> Waiting for report.json ', reportJsonPath);
            return watchForChange(reportJsonPath)
          }
          else {
            console.log('  >>> Running ', executablePath, args);
            return atomLinter
              .exec(executablePath, args, {
                uniqueKey: 'elm make: ' + projectDirectory,
                stream: 'both', // stdout and stderr
        ...
        .then(data => {
          ...
          if (atom.config.get('elmjutsu.reportWarnings') == true) {
            ...
            if (valuesWithoutTypeAnnotations.length > 0) {
              let scanStart = [0, 0];
              const namesAndRanges = _.first(valuesWithoutTypeAnnotations, 20)
                .map(({ name }) => {
                  let issueRange = null;
  ...
  getInferredTypeAnnotations(
    ...
    const proc = evalHelper.executeElmRepl(
      ...
      numOutputsNeeded,
      (errString, outString) => {
        // console.log('namesAndRanges', namesAndRanges, outString);
        // Delete temp file.
...
function formatIssue(usingLinterV1, title, filePath, message, range, severity) {
  // console.log('usingLinterV1', usingLinterV1);
